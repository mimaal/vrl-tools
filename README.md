...
The status bar says which of the two answers you are looking at: `VRL 0.35.0`
for an unknown event, `VRL 0.35.0 · sample` when a sample is in use. Editing
...
v0.5.0, on the
[Marketplace](https://marketplace.visualstudio.com/items?itemName=miguel-martinez.vrl-tools),
and feature-complete against the plan. Highlighting, snippets, config
injection, compiler diagnostics, hover, completion, signature help,
sample-typed checking and running a program against a sample event all work.

Releases are built by GitHub Actions from a tag, and every one attaches its
`.vsix` to the GitHub release for anyone who would rather install it by hand.

Pinned to the `vrl` crate `0.35.0`, which is what Vector 0.58.0 depends on.
...
- `cargo about`, which generates the third-party licence notices that ship in
  the `.vsix`: `cargo install cargo-about --locked --features cli`. It is part
  of `npm run build` on purpose — notices that are regenerated only when
  somebody remembers are notices that are wrong.
...
```sh
npm install
npm run build        # grammar, notices, the wasm module, the extension
...
```sh
code --install-extension editors/vscode/vrl-tools-0.5.0.vsix
```
...
so its terms apply to the crate's own files only, and its source is at
<https://github.com/vectordotdev/vrl> under the pinned tag.

The extension ships that crate and its dependency tree compiled to
WebAssembly, so the terms of all 243 of them travel with it, in
[`editors/vscode/THIRD-PARTY-NOTICES.md`](editors/vscode/THIRD-PARTY-NOTICES.md).
That file is generated by `npm run gen:notices` from the dependency graph that
actually ships, and the generation fails on any licence not listed in
`about.toml`.
